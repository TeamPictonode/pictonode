...
# Runs prettier on all JS projects in the repo and runs flake8 on all Python projects in the repo
...
do
  ...
  npx prettier --check $project
done

# Run flake8 on all Python projects
PY_PROJECTS=(
  "gimp/gtk-nodes-init"
...
then
  echo "flake8 could not be found. Please install it with 'pip install flake8'"
  exit
fi
...
do
  ...
  # List all .py files in the project and run flake8 on them
