...
describe("Result", () => {
    ...
    it("should be able to match an Ok", () => {
        const result = Ok(5);
        expect(result.match((value) => value, (error) => error)).to.equal(5);
    });

    it("should be able to match an Err", () => {
        const result = Err("error");
        expect(result.match((value) => value, (error) => error)).to.equal("error");
    });
    ...
    it("should andThen an Ok and an Err", () => {
        const result = Ok(5);
        expect(() => result.andThen(_ => Err("error")).unwrap()).to.throw("error");
    });

    it("should andThen an Err and an Ok", () => {
        const result = Err("error");
        expect(() => result.andThen((value: number) => Ok(value + 1)).unwrap()).to.throw("error");
    });
...
describe("Option", () => {
    ...
    it("should be able to match a Some", () => {
        const option = Some(5);
        expect(option.match((value) => value, () => 10)).to.equal(5);
    });

    it("should be able to match a None", () => {
        const option = None();
        expect(option.match((value) => value, () => 10)).to.equal(10);
    });
